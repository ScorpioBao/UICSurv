...
class sNet(nn.Module):
    def __init__(self, dim, cls_num) -> None:
        super().__init__()
        # 第一层
        self.conv1 = nn.Sequential(
            nn.Conv3d(1, dim // 4, kernel_size=(3, 3, 3), padding=1),  # 添加 padding=1
            nn.GroupNorm(8, dim // 4),
            nn.ReLU(),
            nn.Conv3d(dim // 4, dim // 4, kernel_size=(3, 3, 3), padding=1),  # 添加 padding=1
            nn.GroupNorm(8, dim // 4),
            nn.ReLU(),
        ...
        self.conv2 = nn.Sequential(
            nn.Conv3d(dim // 4, dim // 2, kernel_size=(3, 3, 3), padding=1),  # 添加 padding=1
            nn.GroupNorm(8, dim // 2),
            nn.ReLU(),
            nn.Conv3d(dim // 2, dim // 2, kernel_size=(3, 3, 3), padding=1),  # 添加 padding=1
            nn.GroupNorm(8, dim // 2),
            nn.ReLU(),
            nn.MaxPool3d(2, stride=2)
        )
        ...
        self.conv3 = nn.Sequential(
            nn.Conv3d(dim // 2, dim, kernel_size=(3, 3, 3), padding=1),  # 添加 padding=1
            nn.GroupNorm(8, dim),
            nn.ReLU(),
            nn.Conv3d(dim, dim, kernel_size=(3, 3, 3), padding=1),  # 添加 padding=1
            nn.GroupNorm(8, dim),
            nn.ReLU(),
            nn.MaxPool3d(2, stride=2)
        )
        # 自适应池化层
        self.adaptive_pool = nn.AdaptiveAvgPool3d((1, 1, 8))  # 自适应池化
        # 全连接层
        self.linear_classification = nn.Sequential(
            nn.Linear(dim * 8, cls_num),  # 全连接层
        )
    ...
    def forward(self, mri):
        ...
        # 自适应池化
        pooled_out = self.adaptive_pool(conv3_out)
        # 展平
        pooled_out = pooled_out.reshape(pooled_out.shape[0], -1)  # batch, feature_dim
        # # 分类
        scores = self.linear_classification(pooled_out)
        return scores
    
    

class HealNetSNet(nn.Module):
    def __init__(
        self,
        *,
        n_modalities: int,
        ...
        # self.T2_UNETR = UNETR_T2(in_channels=1, out_channels=1, img_size=(256,256,20)).cuda()
        # self.DWI_UNETR = UNETR_DWI(in_channels=1, out_channels=1, img_size=(126,126,10)).cuda()
        self.T2_Snet = sNet(dim=128, cls_num=128).cuda()
        self.DWI_Snet = sNet(dim=128, cls_num=128).cuda()
        ...
        for i in range(len(tensors)):
            if i in missing_idx: 
                ...
                if tensors[i].shape[2] == 256 :
                    ...
                    feature= self.T2_Snet(tensors[i])
                    tensors[i] = feature
                    # 添加一个维度
                    ...
                    feature= self.DWI_Snet(tensors[i])
                    tensors[i] = feature
                    # 添加一个维度
                    tensors[i] = tensors[i].unsqueeze(1)
