...
class model_T2(nn.Module):
    def __init__(self, num_output_features=64, frozen_layers=-1):
        ...
        # 替换最后的全连接层以适应我们的特征维度
        self.resnet.fc = nn.Linear(self.resnet.fc.in_features, num_output_features)
    ...
    def extract_resnet_features(self, x):
        # batch,1,256,256,20 转为 batch,256,256,20
        x = x.squeeze(1)
        batch_size, height, width, depth = x.size()
        x = x.permute(0, 3, 1, 2)
        x = self.convert_to_rgb(x)  # 转换为三通道
        x = x.reshape(batch_size * depth, 3, height, width)

        # 提取特征
        with torch.no_grad():
            features = self.resnet(x)

        # 将特征重新调整为 (batch_size, depth, num_output_features)
    ...
    def convert_to_rgb(self, mri_image):
        # 将单通道 MRI 图像映射到三通道（伪彩色）
        r_channel = mri_image
        g_channel = mri_image * 0.8
        b_channel = mri_image * 0.6
        return torch.stack([r_channel, g_channel, b_channel], dim=1)
    ...
    def freeze_resnet_layers(self, frozen_layers=-1):
        if frozen_layers == 0:
            ...
            layers_to_freeze = list(self.resnet.children())[:frozen_layers]
            for layer in layers_to_freeze:
                for param in layer.parameters():
                    param.requires_grad = False
            for layer in list(self.resnet.children())[frozen_layers:]:
                for param in layer.parameters():
                    param.requires_grad = True 



class model_DWI(nn.Module):
    def __init__(self, num_output_features=64, frozen_layers=-1):
        super(model_DWI, self).__init__()
...
class HealNet_U_Site_ResNet(nn.Module):
    def __init__(
        ...
        *,
        ...
        self.T2_UNETR = model_T2(num_output_features=64,frozen_layers=-1).cuda()
        self.DWI_UNETR = model_DWI(num_output_features=64,frozen_layers=-1).cuda()
