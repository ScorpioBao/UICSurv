...
logging.basicConfig(
    ...
    handlers=[
        #logging.FileHandler(f'./logs/training_log_{mod}.txt',mode=mod),  # 将日志保存到文件
...
# 训练函数
def train_model(model, train_loader, optimizer, loss_fn, device, epoch_index, num_epochs, uncertainty_dict,fold_num):
    model.train()
    running_loss = 0.0
    for i, (t2, dwi, tab, target_c, target, d, id, site) in enumerate(train_loader):
        ...
        loss_batch = loss_fn(log_hazards_batch, Y_batch, D_batch, evidence_sum, probability, epochs=epoch_index) +  0.0 * c_loss
        optimizer.zero_grad()
        loss_batch.backward()
        optimizer.step()
...
def main():
    ...
    table_dir = f'./data/cv_folds1'
    mri_dirs = {
        'beizhong_t2': '/home/bqs/Dataset/nii.gz文件/8.2npy/beizhong_T2_registered',
        'fuzhong_t2': '/home/bqs/Dataset/nii.gz文件/8.2npy/fuzhong_T2_registered2',
        'SHANXI_t2': '/home/bqs/Dataset/nii.gz文件/8.2npy/SHANXI_T2_registered',
        'beizhong_dwi': '/home/bqs/Dataset/nii.gz文件/8.2npy/beizhong_DWI_registered',
        'fuzhong_dwi': '/home/bqs/Dataset/nii.gz文件/8.2npy/fuzhong_DWI_registered2',
        'SHANXI_dwi': '/home/bqs/Dataset/nii.gz文件/8.2npy/SHANXI_DWI_registered',
        'shanxi_t2': '/home/bqs/Dataset/nii.gz文件/8npy/SHANXI_T2_40000',
    }
    ...
    for fold_num in range(1, 6):
        ...
        for epoch_index in range(num_epochs):
            ...
            epoch_loss = train_model(model, train_loader, optimizer, deephit_loss_uncertainty, device, epoch_index, num_epochs, uncertainty_dict,fold_num)
            logging.info(f"Epoch {epoch_index + 1} - Average Loss: {epoch_loss:.4f}")

            train_c_index, train_surv, all_train_labels, all_train_labels_c, all_train_D = evaluate_model_wo_missing(model, train_loader, device, batch_size, time_grid_train_np)
            val_c_index, val_surv, all_val_labels, all_val_labels_c, all_val_D = evaluate_model_wo_missing(model, val_loader, device, batch_size, time_grid_train_np)
            test_c_index, test_surv, all_test_labels, all_test_labels_c, all_test_D = evaluate_model_wo_missing(model, test_loader, device, batch_size, time_grid_train_np)
            ...
            if  test_c_index > best_C_index:
                ...
                epochs_without_improvement = 0
                #torch.save(model.state_dict(), f'./checkpoints_wo_missing/{site}_fold_{fold_num}_best_model_{mod}.pth')
                torch.save(model.state_dict(), f'./checkpoints_wo_missing/fold_{fold_num}_best_model_{mod}.pth')
