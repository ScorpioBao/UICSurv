...
class LogisticHazard(models.base.SurvBase):
    ...
    def __init__(self, net, optimizer=None, device=None, duration_index=None, loss=None, act=True):
        self.duration_index = duration_index
        if loss is None:
            loss = models.loss.NLLLogistiHazardLoss()
        else:
            loss = models.loss.NLLLogistiHazardLoss_uncertainty()
        self.sigmoid = act
    ...
    def predict_surv_df(self, input, batch_size=256, eval_=True, num_workers=0):
    ...
    def predict_surv(self, input, batch_size=256, numpy=None, eval_=True, to_cpu=False,
    ...
    def predict_hazard(self, input, batch_size=256, numpy=None, eval_=True, to_cpu=False,
                       num_workers=0):
        if self.sigmoid:
            hazard = self.predict(input, batch_size, False, eval_, False, to_cpu, num_workers).sigmoid()
        else:
            hazard = self.predict(input, batch_size, False, eval_, False, to_cpu, num_workers)
            inter = hazard.shape[1]//2
            alpha = hazard[:, :inter]
            beta = hazard[:, inter:]
            S = alpha + beta
            hazard = alpha.div(S)
