...
def binary_cross_entropy_manual(z, y, epsilon=1e-10):
    """
    手动实现二元交叉熵损失，其中 z 是概率值。

    参数:
    - z: torch.Tensor, 模型输出的概率值 (batch_size, num_intervals)
    - y: torch.Tensor, 真实标签 (batch_size, num_intervals), 值为 0 或 1
    - epsilon: float, 用于防止 log(0) 的数值稳定性处理

    返回:
    - bce_loss: torch.Tensor, 每个样本的交叉熵损失，形状为 (batch_size, num_intervals)
    """
    # 确保 z 的值在 (0, 1) 范围内，使用 epsilon 避免 log(0) 的情况
    z = torch.clamp(z, epsilon, 1.0 - epsilon)

    # 计算二元交叉熵损失
    bce_loss = (y * torch.log(z) + (1 - y) * torch.log(1 - z))
...
def nll_logistic_hazard_uncertainty(phi: Tensor, idx_durations: Tensor, events: Tensor,
                        reduction: str = 'mean') -> Tensor:
    """Negative log-likelihood of the discrete time hazard parametrized model LogisticHazard [1].
    
    Arguments:
        phi {torch.tensor} -- Estimates in (-inf, inf), where hazard = (phi). alpha and beta are the uncertainty parameters.
        idx_durations {torch.tensor} -- Event times represented as indices.
        events {torch.tensor} -- Indicator of event (1.) or censoring (0.).
            Same length as 'idx_durations'.
        reduction {string} -- How to reduce the loss.
    ...
    # phi_logit = phi
    # phi = F.softplus(phi)
    num_intervals = phi.shape[1] // 2
    alpha = phi[:, :num_intervals]  # 取前 num_intervals 个作为 alpha
    beta = phi[:, num_intervals:]   # 取后 num_intervals 个作为 beta
    S = alpha + beta
    ...
    events = events.view(-1, 1)
    idx_durations = idx_durations.view(-1, 1)
    y_bce = torch.zeros_like(pro).scatter(1, idx_durations, events)
    
    # bce = binary_cross_entropy_manual(pro, y_bce)
    # bce = F.binary_cross_entropy_with_logits(z, y_bce, reduction='none')
    bce = F.binary_cross_entropy(pro, y_bce, reduction='none')
    loss = bce.cumsum(1).gather(1, idx_durations).view(-1)
    return _reduction(loss, reduction)
...
def cox_ph_loss_sorted(log_h: Tensor, events: Tensor, eps: float = 1e-7) -> Tensor:
    ...
    if events_sum > 0:
        ...
        return - log_h.sub(log_cumsum_h).mul(events).sum().div(events_sum)
    else:
        return torch.tensor(0, dtype=torch.float, device=log_h.device)
...
class NLLLogistiHazardLoss_uncertainty(_Loss):
    """Negative log-likelihood of the hazard parametrization model.
    See `loss.nll_logistic_hazard` for details.
    ...
    """
    def forward(self, phi: Tensor, idx_durations: Tensor, events: Tensor) -> Tensor:
        return nll_logistic_hazard_uncertainty(phi, idx_durations, events, self.reduction)
